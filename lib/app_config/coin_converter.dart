...
Color? coinBaseColor(String coinTicker) {
  final isAbbreviation = appConfig.protocolSuffixes.any(
    (suffix) => coinTicker.toUpperCase().contains(suffix.toUpperCase()),
  );

  String? parsedTicker = coinTicker;

  if (isAbbreviation) {
    parsedTicker = getCoinTicker(coinTicker);
  }

  if (parsedTicker == null) return null;

  return Color(int.parse(_allColors[parsedTicker]!.replaceAll('#', '0xFF')));
}

Map<String, String> _allColors = {
  '1INCH': '#95A7C5',
  'AAVE': '#9C64A6',
  'ABY': '#8B0D10',
  'ABY-OLD': '#8B0D10',
...
String _getColor(String coin) {
  ...
  return '0xFF${_allColors[getCoinTicker(coin)]?.replaceAll('#', '') ?? defaultColor}';
